function [sd] = Test_single_PolyDecay(n,r,R,p,field,model)
%This script runs an experimental instance using a matrix with polynomial 
%decay in the spectrum. See our reference paper for more details.
%	Inputs: n 		-> size of input matrix (n x n)
%			r		-> target rank
%			R		-> effective rank
%			p		-> parameter that controls the rate of decay
...
singVals(1:R) = 1;
singVals((R+1):n) = (((1:(n-R))+1).^(-p));
A = diag(singVals+0*1i);
